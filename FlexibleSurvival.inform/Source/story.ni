...
[itemcheatm is an action applying to one topic.
understand "itemcheatm [text]" as itemcheatm.
	
carry out itemcheatm:
	let numberToAdd be "10";
	let tempitem be text;
	if tempitem matches the regular expression "^\d+\b":
		replace the text "[text matching regular expression]" in tempitem with "";
		let numberToAdd be "[text matching regular expression]";
	repeat with x running through grab objects:
		if the printed name of x exactly matches the text topic understood, case insensitively:
			increase carried of x by "[numberToAdd]";
			break;
]
